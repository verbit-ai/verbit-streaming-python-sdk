...
_Note: phrasings and word order in translated responses may be significantly different from those of the source language, due to the nature of translation to another language. Since the translated words were never really uttered, they do not have "real" timings. However, words in "translation" responses are assigned timings which are heuristically distributed within the time boundaries of the source language utterance, as these timings may be useful for some purposes (e.g. displaying translated captions in sync with the media)._ 
