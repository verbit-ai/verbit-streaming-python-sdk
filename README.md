...
_Note: words in responses from the "translation" service type are assigned timings which are heuristically distributed across the time-spans of their counterpart words in the source language, as these timings may be useful for some purposes (e.g. displaying translated captions in sync with the media)._ 
